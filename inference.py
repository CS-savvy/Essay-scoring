...
class GraphInference:
    ...
    def infer(self, graph_ids: list, output_dir: Path) -> None:
        results = []
        for graph_id in tqdm(graph_ids, desc="Processing .."):
            ...
            results.append(",".join([graph_id, str(original_score), str(pred_score),  " ".join(texts).replace(',', '..')]))

        with open(output_dir / "inference_results.csv", 'w') as f:
            f.write("\n".join(results))
        return
